...
def generate_daily_report(
    ...
    """
    Generate daily performance report with broker capital as primary source.

    Args:
        trades: List of trade dictionaries
        broker_capital_start: Broker capital at day start (ACTUAL from broker)
        broker_capital_end: Broker capital at day end (ACTUAL from broker)
        broker_brokerage: Total brokerage charges for the day
    ...
    if broker_capital_start is not None:
        ...
        logging.info("📊 Using BROKER start capital: ₹%.2f", initial_capital)
    else:
    ...
    system_equity = initial_capital + total_pnl

    # ✅ USE BROKER END CAPITAL AS PRIMARY
    if broker_capital_end is not None:
        final_equity = broker_capital_end
        logging.info("📊 Using BROKER end capital: ₹%.2f", final_equity)
    else:
        final_equity = system_equity
        logging.info("📊 Using SYSTEM calculated equity: ₹%.2f", final_equity)

    # Calculate equity curve for drawdown (using system PnL)
    equity = initial_capital
    max_dd = 0

    for trade in trades:
        equity += trade.get("pnl", 0)
        dd = initial_capital - equity
        max_dd = max(max_dd, dd)

    max_dd_pct = (max_dd / initial_capital * 100) if initial_capital > 0 else 0

    # Calculate broker vs system variance
    ...
    broker_pnl = broker_capital_diff if broker_capital_diff is not None else None
    variance = None
    if broker_pnl is not None:
        variance = broker_pnl - system_pnl
    ...
    return {
        ...
        "avg_trades_per_day": total_trades,
        "max_trades_in_a_day": total_trades,
        "min_trades_in_a_day": total_trades,
        "win_rate": round(win_rate, 4),
        "total_pnL": round(total_pnl, 2),
        "max_drawdown": round(max_dd, 2),
        "max_drawdown_pct": round(max_dd_pct, 2),
        "max_lots_used": max(
            (t.get("lots", 1) for t in trades), default=STRATEGY["initial_lots"]
        ),
        "open_positions": 0,
...
class TradeExecutor:
    ...
    def __init__(self, strategy, data_engine, order_manager=None):
        ...
        self._optionchain_cache = None
        self._optionchain_cache_time = 0
        self._optionchain_cache_ttl = 60
    ...
    def execute_entry(
        ...
        from src.utils import get_synthetic_future_symbols

        symbols = get_synthetic_future_symbols(nifty_price, action, expiry)

        buy_symbol = symbols["buy_symbol"]
        sell_symbol = symbols["sell_symbol"]

        buy_data = self._get_option_data(buy_symbol)
        sell_data = self._get_option_data(sell_symbol)

        if not buy_data or not sell_data:
            logging.error("❌ Failed to get option data for synthetic future")
            return None
        ...
        self.buy_entry_oi = buy_data.get("oi", 0)
        self.buy_entry_volume = buy_data.get("volume", 0)
        self.sell_entry_oi = sell_data.get("oi", 0)
        self.sell_entry_volume = sell_data.get("volume", 0)
    ...
    def _get_option_data(self, symbol: str) -> Optional[Dict[str, Any]]:
        """Get option price, OI, Volume from optionchain API."""
        current_time = time_module.time()
        if (current_time - self._optionchain_cache_time) < self._optionchain_cache_ttl:
            if self._optionchain_cache:
                cached_data = self._find_option_in_cache(symbol)
                if cached_data:
                    return cached_data

        try:
            expiry_timestamp = self._get_expiry_timestamp_from_fyers()
            ...
            if not response or response.get("code") != 200:
                logging.error("❌ Optionchain API failed")
                return None

            self._optionchain_cache = response.get("data", {})
            self._optionchain_cache_time = current_time

            return self._find_option_in_cache(symbol)

        except Exception as exc:
            logging.error("❌ Optionchain fetch error: %s", exc)
            return None
...
class Trader:
    ...
    def _init_components(self):
        ...
        # Verify position with broker if LIVE mode and state shows open position
        if TRADING_MODE == "LIVE" and self.order_manager and self.position != 0:
            logging.info("=" * 80)
        ...
        # Load warmup data and build initial bricks (ONE TIME)
        ...
        # Record broker capital at start
    ...
    def _load_state(self):
        """Load previous state from file and sync with executor."""
        state = load_state()
        if not state:
            return
        ...
        # Recover entry_option_price from buy_entry_price if missing
        ...
        # Load synthetic position into executor
        ...
        if self.position != 0:
            direction = "LONG" if self.position == 1 else "SHORT"
            print(f"\n{'=' * 80}")
            print(f"💼 RESUMED {direction} POSITION ({self.current_lots} lots)")
            ...
            if self.buy_symbol and self.sell_symbol:
                ...
                print(f"   📊 {self.option_symbol} @ ₹{self.entry_option_price:.2f}")
    ...
    def _process_minute(self, current_time: datetime):
        """Process each minute of trading."""

        # Position reconciliation (every 5 minutes)
        if current_time.minute % 5 == 0 and current_time.second < 10:
            if self.order_manager and TRADING_MODE == "LIVE":
                try:
                    ...
                    if mismatches:
                        ...
                        for symbol, (internal, broker) in mismatches.items():
                            if (
                                ...
                                logging.error("🚨 Forcing position to FLAT")
                                self.position = 0
                                self.entry_info = None
                                self.option_symbol = None
                                self.entry_option_price = None
                                ...
                                self._save_state()
                                send_system_alert(
                                    "error", "Position mismatch - forced FLAT"
                                )
                except Exception as exc:
        ...
        # Check expiry day exit (only on expiry day)
        should_exit, reason = check_expiry_exit(self.position, expiry)
        if should_exit:
            logging.warning("⚠️ Expiry exit: %s", reason)
            self._force_exit(current_time, reason)
            return

        # Check if hedge should be bought (3:25 PM)
        ...
        # Process strategy FIRST to check for exit signals
        ...
        # If NO exit signal → Check if hedge should exit alone at 9:16 AM
        if not exit_signal_generated:
            ...
            if should_exit_hedge:
                logging.info("🛡️ Exiting hedge only (no exit signal): %s", hedge_reason)
                self._exit_hedge_only(current_time, hedge_reason)

        # Handle signal (entry or exit)
        # If it's an EXIT signal, it will close complete trade (main + hedge together)
        if signal_result:
    ...
    def _process_trade_result(self, result: Dict[str, Any], action: str):
        ...
        Don't clear hedge state on main position exit if hedge_exit_pending=True.

        Sets option_symbol from buy_symbol for tracking.
        Clears executor state on exit.
        """
        ...
        if action == "EXIT":
            ...
            # Only exit hedge if NOT pending next-day exit
            if self.hedge_bought and self.hedge_symbol and not self.hedge_exit_pending:
                logging.info("🛡️ Exiting hedge after main position exit (same-day)")
                self._exit_hedge_only(get_current_time(), "Main position exited")
            elif self.hedge_exit_pending:
                logging.info("🛡️ Keeping hedge for next-day exit at 9:16 AM")

            # Reset main position state (but keep hedge if pending)
            self.position = 0
            self.entry_info = None
            self.option_symbol = None
            self.entry_option_price = None
            ...
            # Set option_symbol for tracking/display
    ...
    def _exit_hedge_only(self, current_time: datetime, reason: str) -> None:
        """
        Exit hedge only (keep main position open).

        Called next day at 9:16 AM if no exit signal.

        Always clears hedge state after exit to prevent stuck state.
        ...
        logging.info("🛡️ Exiting hedge: %s", reason)

        try:
            # Get current hedge price
            quotes = self.data_engine.get_quotes([self.hedge_symbol])
            hedge_exit_price = self.hedge_entry_price
            ...
            if TRADING_MODE == "LIVE" and self.order_manager:
                ...
                if exit_order.is_complete or exit_order.is_partial:
                    ...
                    # Calculate hedge PnL
                    hedge_pnl = (
                        (filled_price - self.hedge_entry_price)
                        * LOT_SIZE
                        * self.current_lots
                    )
                    ...
                    print(f"\n{'=' * 80}")
                    print(f"🛡️ HEDGE EXITED (Position Continues)")
    ...
    def _force_exit(self, current_time: datetime, reason: str):
        """
        Force exit position (including hedge if present).

        Only exits hedge if it's NOT a next-day scenario.
        If hedge_exit_pending=True, keep hedge for next day exit at 9:16 AM.
        ...
        try:
            ...
            if self.hedge_bought and self.hedge_symbol:
                if not self.hedge_exit_pending:
                    ...
                    should_exit_hedge_now = True
                    logging.info("🛡️ Exiting hedge due to emergency/expiry")
                else:
            ...
            if trade_result:
                ...
                logging.info("Forced exit completed: %s", reason)
            else:
                logging.error("Forced exit failed!")
    ...
    def cleanup(self):
        ...
        if self.position != 0:
            logging.warning("Position open at shutdown - forcing exit")
            self._force_exit(get_current_time(), "System shutdown")

        # Fetch broker capital at end of day
        ...
        if trades:
            ...
            report = _empty_report()
            report["broker_capital_start"] = self.broker_capital_start
            report["broker_capital_end"] = self.broker_capital_end
            report["broker_brokerage"] = self.broker_brokerage_today
            save_daily_report(report)
            send_daily_summary(report)
